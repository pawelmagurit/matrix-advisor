...
The system SHALL represent cutting orders as `OrderLine` records with fields: `orderId`, `profileCode`, `alloy`, `lengthMm`, `quantity`, optional `tolerancePlusMm` (default 10), `toleranceMinusMm` (default 0), and optional `priority`.
...
The system SHALL ship a built-in sample dataset: profile `E-08421`, alloy `6060`, kg/m 1.2, stock 6500 mm, kerf 4 mm, minOffcutReusable 200 mm, with five orders (ZL-101 through ZL-105) as defined in the project prompt.

#### Scenario: One-click sample load

- **WHEN** the user clicks "Wczytaj przykład Extral"
- **THEN** the system SHALL populate orders and config with the sample dataset

### Requirement: CSV import

The system SHALL accept CSV upload mapping to `OrderLine` fields (`orderId`, `profileCode`, `alloy`, `lengthMm`, `quantity` at minimum).
